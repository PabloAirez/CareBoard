...
interface ActiveDemandListProps {
  demands: Call[];
  onResolve: (demandId: string) => void;
}
...
export function ActiveDemandList({ demands, onResolve }: ActiveDemandListProps) {
  if (demands.length === 0) {
    return (
    ...
    <section className="space-y-2">
      {demands.map((demand) => (
        ...
        >
          ...
          <button
            type="button"
            onClick={() => onResolve(demand.id)}
            className="shrink-0 rounded-md bg-secondary px-3 py-2 text-xs font-black text-white"
          >
            Atendida
          </button>
        </article>
